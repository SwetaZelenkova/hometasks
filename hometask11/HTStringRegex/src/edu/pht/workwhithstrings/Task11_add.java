...
public class Task11_add {//с цифрами чтоб понять, что как должно располагаться
	...
	public static void main(String[] args) {
		...
		char[][] ch = { { 'I', 'l', 'o', 'v' }, { 'e', 'l', 'i', 'f', }, { 'e', 'a', 'n', 'd' },
				{ 'r', 'e', 's', 't' } };
		for (int i = 0; i < ch.length; i++) {
			for (int j = 0; j < ch[i].length; j++) {
		...
		for (int j = 0, d = n; j <= d; j++, d--) {
			for (int i = j; i < d; i++) {
				System.out.printf("%c ", ch[j][i]);
			}
			for (int i = j + 1; i < d; i++) {
				System.out.printf("%c ", ch[i][n - j - 1]);
			}
			for (int i = j + 1; i < d; i++) {
				System.out.printf("%c ", ch[n - j - 1][n - i - 1]);
			}
			for (int i = j + 1; i < d - 1; i++) {
				System.out.printf("%c ", ch[n - i - 1][j]);
			}
