...
public class Page {
	...
	public LinkedHashSet<Dropdown> getElementsDropdown() throws NoSuchEntityException {
		int length = SET.length();
		int thirdElement = 3;
		LinkedHashSet<Dropdown> setDropdowns = new LinkedHashSet<>(dropdowns);
		String className = dropdowns.getClass().getName();
		String type = className.substring(className.length() - length).toLowerCase();
		...
		if (type.equals(SET)) {
			...
			if (dropdowns != null && dropdowns.size() >= thirdElement) {
				...
				System.out.println("dropdowns is empty or contains less than three elements");
			}
		}
		return setDropdowns;
	}
